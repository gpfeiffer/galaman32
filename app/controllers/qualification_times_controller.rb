class QualificationTimesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @qualification_time.save
        format.html { redirect_to(@qualification_time, :notice => 'Qualification time was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @qualification_time.update_attributes(params[:qualification_time])
        format.html { redirect_to(@qualification_time, :notice => 'Qualification time was successfully updated.') }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(qualification_times_url) }
      format.xml  { head :ok }
    end
  end
end
